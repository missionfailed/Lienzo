...
'''guarda en el registro de memoria apuntado por variable el valor de la respuesta, dentro del ambiente de ejecucion 
(el tope de la pila'''
...
def executeVM(dirProc, listaCuadruplos):
    ...
    c = listaCuadruplos[i]
    op = c[0]
    # mientras que el cuadruplo no sea el FIN
    while op != END:
...
'''metodo que regresa el tipo del valor que esta en el cuadruplo, dependiendo de la instancia que es'''
def Tipo(valor1):
